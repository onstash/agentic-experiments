...
import { runOpportunityAgent } from "../../typescript-agent/src/agents/agent.js";

async function main() {
  const args = process.argv.slice(2);
  const fileIndex = args.indexOf("--profile");
  const jsonIndex = args.indexOf("--profile-json");
  const profile = fileIndex >= 0 && args[fileIndex + 1]
  ...
  if (profile) {
    ...
    if (query) console.log(JSON.stringify(await runOpportunityAgent(profile, query), null, 2));
