...
export async function runAgenticOpportunitySearch(
  ...
  options: {
    maxQueries: number;
    metadata?: Partial<AgentRunMetadata>;
  ...
  const queries = query?.trim()
    ? [{ priority: 1, query: query.trim(), reason: "User supplied query." }]
    : deriveQueries(profile, options.maxQueries);
  ...
  emit({
    ...
    reason: query ? "Using the user query." : "Derived queries from the profile.",
  });
  ...
  for (const [index, planned] of queries.entries()) {
    ...
    try {
      ...
      emit({
        ...
        iteration,
        count: ranked.length,
        reason: "Search failed after earlier results were collected.",
      });
      break;
    }
    found.push(...results);
  ...
  if (dependencies.recommend) {
    ...
    recommendation = await dependencies.recommend(
      profile,
      recommendationQuery,
